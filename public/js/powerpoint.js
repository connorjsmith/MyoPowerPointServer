...
$(document).ready(function() {
  ...
  function prevLink() {
    var previous = $('.focused-item').prevAll('a').last();
  ...
  function nextLink() {
    var next = $('.focused-item').nextAll('a').first();
  ...
  socket.on('connect', function() {
    ...
    socket.on('close link', function() {
        document.getElementById("link").style.display = "none";
        $('.focused-item').removeClass('focused-item');
        slideMode();
    });
    socket.on('link show', function(url) {
        // remeber to get the link
